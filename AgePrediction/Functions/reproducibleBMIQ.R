# BMIQ function from the wateRmelon package with an added seed value to ensure stable results when rerunning the
# function

# Changes made by Roy Lardenoije, 2020-02-14

reproducibleBMIQ <- function (beta.v, nL = 3, doH = TRUE, nfit = 5000, th1.v = c(0.2, 0.75), 
                              th2.v = NULL, niter = 5, tol = 0.001, plots = FALSE, pri = FALSE, 
                              seed = 707) 
  {
    ...
    internalBMIQ <- function (beta.v, design.v, nL = 3, doH = TRUE, nfit = 50000, th1.v = c(0.2, 0.75), 
                              th2.v = NULL, niter = 5, tol = 0.001, plots = TRUE, 
                              sampleID = 1, pri = TRUE, seed = 707) 
    {
      ...
      #set.seed(seed)
      rand.idx <- sample(1:length(beta1.v), nfit, replace = FALSE)
      em2.o <- blc(matrix(beta2.v[rand.idx], ncol = 1), w = w0.m[rand.idx,], maxiter = niter, 
                   tol = tol, verbose = pri)
      print("Done")
      ...
      if (plots) {
        ...
        legend(x = 0.5, y = ymax, legend = c("type1", "type2", "type2-BMIQ"), bty = "n", 
      ...
      out[good] <- pnbeta.v
      pnbeta.v <- out
      return(list(nbeta = pnbeta.v, class1 = class1.v, class2 = class2.v, av1 = classAV1.v, 
                  av2 = classAV2.v, hf = hf, th1 = nth1.v, th2 = th2.v))
    }
    
    betas <- sapply(colnames(ibetas), function(name) {
      ou <- try(internalBMIQ(ibetas[, name], design.v = d, nL, doH, nfit, th1.v, th2.v, niter, tol, 
                             plots, sampleID = name, pri = FALSE, seed = seed))
    ...
    object@history <- rbind(object@history, data.frame(submitted = history.submitted, 
